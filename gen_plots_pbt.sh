...
for ic in "${IC_CONFIGS[@]}"; do
    for id in "${TRIAL_IDS[@]}"; do
        ...
        mkdir -p "logs/episodes/alfworld/train/rap_flex/openai/gpt-4o-mini/pbt_combined_${ic}_trial_${id}/"
        ...
        for segment in "${SEGMENTS[@]}"; do
            ...
            if [ -d "$src_dir" ]; then
                cp "$src_dir"/*.txt "logs/episodes/alfworld/train/rap_flex/openai/gpt-4o-mini/pbt_combined_${ic}_trial_${id}/"
            fi
        done
...
for ic in "${IC_CONFIGS[@]}"; do
    for task_type in "${TASK_TYPES[@]}"; do
        ...
        folder_paths=""
        for id in "${TRIAL_IDS[@]}"; do
            folder_paths+="logs/episodes/alfworld/train/rap_flex/openai/gpt-4o-mini/pbt_combined_${ic}_trial_${id}/ "
        done
        ...
        for id in "${TRIAL_IDS[@]}"; do
            ...
            if [ "$task_type" = "substep" ]; then
                python scripts/plot_from_folder.py logs/episodes/alfworld/train/rap_flex/openai/gpt-4o-mini/pbt_combined_${ic}_trial_${id}/ --task_type $task_type --granularity 200
            else
                python scripts/plot_from_folder.py logs/episodes/alfworld/train/rap_flex/openai/gpt-4o-mini/pbt_combined_${ic}_trial_${id}/ --task_type $task_type --granularity 50
            fi
        done
    done
    ...
    for id in "${TRIAL_IDS[@]}"; do
        python scripts/plot_from_folder.py logs/episodes/alfworld/train/rap_flex/openai/gpt-4o-mini/pbt_combined_${ic}_trial_${id}/ --granularity 200
    done
