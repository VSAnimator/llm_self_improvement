...
INITIAL_FOLDER=$2                        # Initial folder to copy from
NUM_IC=$3                                # Number of in-context examples
...
LOG_FILE="$CURRENT_DIR/pbt_${ENV_NAME}_${PBT_RUN_LABEL}_log.txt"
touch "$LOG_FILE"
...
log "Starting PBT run for environment: ${ENV_NAME}"
log "Using initial folder: ${INITIAL_FOLDER}"
...
for trial in {1..5}; do
    ...
    if [ ! -d "$CURRENT_DIR/data/${ENV_NAME}_pbt_${PBT_RUN_LABEL}_trial_${trial}" ]; then
        cp -r "$CURRENT_DIR/data/${INITIAL_FOLDER}" "$CURRENT_DIR/data/${ENV_NAME}_pbt_${PBT_RUN_LABEL}_trial_${trial}"
...
while [ $segment -le $NUM_SEGMENTS ]; do
    ...
    for trial in {1..5}; do
        ...
        python scripts/run_agent_v2.py \
            ...
            --db_path "$CURRENT_DIR/data/${ENV_NAME}_pbt_${PBT_RUN_LABEL}_trial_${trial}/learning.db" \
            --store_episodes \
            --env ${ENV_NAME} \
    ...
    for trial in {1..5}; do
        ...
        mkdir -p "$CURRENT_DIR/data/${ENV_NAME}_pbt_${PBT_RUN_LABEL}_backups/trial_${trial}/segment_${segment}"
        
        # Copy the trial data to the backups folder
        cp -r "$CURRENT_DIR/data/${ENV_NAME}_pbt_${PBT_RUN_LABEL}_trial_${trial}/"* \
            "$CURRENT_DIR/data/${ENV_NAME}_pbt_${PBT_RUN_LABEL}_backups/trial_${trial}/segment_${segment}/"
        
        # Also create the checkpoint in the original location for backward compatibility
        #mkdir -p "$CURRENT_DIR/data/${ENV_NAME}_pbt_${PBT_RUN_LABEL}_trial_${trial}/checkpoint.segment_${segment}"
        #cp -r "$CURRENT_DIR/data/${ENV_NAME}_pbt_${PBT_RUN_LABEL}_trial_${trial}/"* \
        #    "$CURRENT_DIR/data/${ENV_NAME}_pbt_${PBT_RUN_LABEL}_trial_${trial}/checkpoint.segment_${segment}/"
    ...
    if [ $best_trial -ne $worst_trial ]; then
        ...
        echo "Replacing trial $worst_trial with a copy of trial $best_trial" >> "$CURRENT_DIR/data/${ENV_NAME}_pbt_${PBT_RUN_LABEL}_trial_${worst_trial}/replacement_log.txt"

        # Copy the best trial's folder to the worst trial
        rm -rf "$CURRENT_DIR/data/${ENV_NAME}_pbt_${PBT_RUN_LABEL}_trial_${worst_trial}"
        cp -r "$CURRENT_DIR/data/${ENV_NAME}_pbt_${PBT_RUN_LABEL}_trial_${best_trial}" \
             "$CURRENT_DIR/data/${ENV_NAME}_pbt_${PBT_RUN_LABEL}_trial_${worst_trial}"
...
ARCHIVE_DIR="$CURRENT_DIR/data/${ENV_NAME}_archives"
mkdir -p "$ARCHIVE_DIR"

# Get current date and time for the archive name
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
ARCHIVE_NAME="${ENV_NAME}_pbt_${PBT_RUN_LABEL}_trials_${TIMESTAMP}.tar.gz"
