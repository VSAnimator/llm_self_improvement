...
class LiteLLMWrapper:
    ...
    async def __generate_raw(
        ...
        try:
            ...
            completion_kwargs = {
                ...
                "mock_response": (
                    input("Human: ") if "human" in self.model.lower() else None
                ),
            ...
            if (
                response_format
            ):  # and any(provider in self.model.lower() for provider in ["openai", "claude", "anthropic", "gemini", "together"]):
    ...
    async def generate_chat(
        self, messages: List[Dict[str, str]], stop: Optional[List[str]] = ["\n"]
    ) -> str:
        """
