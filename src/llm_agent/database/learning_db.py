...
class LearningDB:
    ...
    def __init__(self, db_path: str = "data/learning.db"):
        ...
        self.state_conn = sqlite3.connect(
            os.path.join(os.path.dirname(db_path), "learning_states.db")
        )
        self.state_cursor = self.state_conn.cursor()

        self.rule_conn = sqlite3.connect(
            os.path.join(os.path.dirname(db_path), "learning_rules.db")
        )
        ...
        for field in self.trajectory_indices.keys():
            mapping_path = os.path.join(
                self.index_path, f"trajectory_{field}_id_mapping.json"
            )
        ...
        for field in self.state_indices.keys():
            mapping_path = os.path.join(
                self.index_path, f"state_{field}_id_mapping.json"
            )
            if os.path.exists(mapping_path):
                with open(mapping_path, "r") as f:
        ...
        for field in self.rule_indices.keys():
            mapping_path = os.path.join(
                self.index_path, f"rule_{field}_id_mapping.json"
            )
            if os.path.exists(mapping_path):
                with open(mapping_path, "r") as f:
    ...
    def _save_index(
        self, field: str, index: faiss.IndexFlatIP, is_trajectory: bool = True
    ):
        """
        ...
        mappings = (
            self.trajectory_id_mappings if is_trajectory else self.state_id_mappings
        )
        with open(mapping_path, "w") as f:
            json.dump(mappings[field], f)

    def _compute_top_k_nearest_neighbors_by_avg_distance(
        self, indices, query_embeddings, k
    ):
        """
        ...
        for i in range(len(indices)):
            distances, neighbors = indices[i].search(
                query_embeddings[i].reshape(1, -1), k
            )
        ...
        for i in range(num_queries):
            ...
            for idx in range(len(indices)):
                ...
                worst_distance = distances[
                    -1
                ]  # Worst distance in the top-k of this index
                ...
                for neighbor in candidates:
                    if (
                        len(candidates[neighbor]) < idx + 1
                    ):  # If this neighbor didn't appear in the current index
                        candidates[neighbor].append(worst_distance)

            # Compute average distances for all candidates
            avg_distances = {
                neighbor: np.mean(distances)
                for neighbor, distances in candidates.items()
            }

            # Sort candidates by average distance and select top-k
            sorted_candidates = sorted(
                avg_distances.items(), key=lambda x: x[1], reverse=True
            )
    ...
    def _get_top_k_by_keys(
        ...
        if self.random_trajectory_retrieval:
            ...
            entry_ids = [
                mappings[key_type[0]][str(i)]
                for i in random.sample(range(len(mappings[key_type[0]])), k)
            ]
        ...
        D, I = self._compute_top_k_nearest_neighbors_by_avg_distance(
            [indices[elem] for elem in key_type], key_embeddings, k
        )
    ...
    def store_rule(
        ...
        for field, (value, embedding) in rule_fields.items():
            if value is not None:
                embedding_array = np.frombuffer(embedding, dtype=np.float32).reshape(
                    1, -1
                )
    ...
    def store_episode(
        ...
        category_embedding = (
            self.model.encode([category])[0].tobytes() if category else None
        )
        plan_embedding = self.model.encode([plan])[0].tobytes() if plan else None
        reflection_embedding = (
            self.model.encode([reflection])[0].tobytes() if reflection else None
        )
        summary_embedding = (
            self.model.encode([summary])[0].tobytes() if summary else None
        )
        ...
        for i in range(len(observations) - 1):
            ...
            reasoning_embedding = (
                self.model.encode([reasoning_i])[0].tobytes() if reasoning_i else None
            )
        ...
        curr_count = self.trajectory_cursor.execute(
            "SELECT COUNT(*) FROM trajectories"
        ).fetchone()[0]
        if curr_count % 100 == 0 or (curr_count < 100 and curr_count % 10 == 0):
            ...
            backup_dir = os.path.join(
                os.path.dirname(db_dir), f"{curr_dir_name}_backups", str(curr_count)
            )
    ...
    def get_rules(
        ...
        for rule in rules:
            ...
            for trajectory_id in trajectory_ids:
                self.trajectory_cursor.execute(
                    "SELECT id, environment_id, goal, category, observations, reasoning, actions, rewards, plan, reflection, summary FROM trajectories WHERE id = ?",
                    (trajectory_id,),
                )
            ...
            for state_id in state_ids:
                self.state_cursor.execute(
                    "SELECT id, state, reasoning, action, next_state FROM states WHERE id = ?",
                    (state_id,),
                )
    ...
    def _get_rules_for_id(self, trajectory_id: int) -> List[Dict]:
        ...
        return [
            {"name": r[0], "content": r[1], "context": r[2]}
            for r in self.rule_cursor.fetchall()
        ]
    ...
    def retrieve_similar_entries(
        ...
        if trajectory_key_types:
            ...
            trajectory_ids, trajectory_distances = self._get_top_k_by_keys(
                ...
                + (10 if filtered_environment_id else 0),
            )
            ...
            if outcome:
                ...
                trajectory_ids = [
                    trajectory_ids[i] for i in np.argsort(trajectory_distances)[::-1]
                ]
                trajectory_distances = [
                    trajectory_distances[i]
                    for i in np.argsort(trajectory_distances)[::-1]
                ]
            if filtered_environment_id:
                ...
                trajectory_distances = [
                    ...
                    if self.trajectory_cursor.execute(
                        "SELECT environment_id FROM trajectories WHERE id = ?",
                        (trajectory_ids[i],),
                    ).fetchone()[0]
                    != filtered_environment_id
                ]
                trajectory_ids = [
                    trajectory_ids[i]
                ...
                for trajectory_id in trajectory_ids:
                    self.state_cursor.execute(
                        ...
                        SELECT id, state, state_embedding, reasoning, reasoning_embedding,
                               action, action_embedding, next_state
                        FROM states WHERE trajectory_id = ?""",
                        (trajectory_id,),
                    )
                    ...
                    for state_row in state_rows:
                        ...
                        for state_key_type in state_key_types:
                            ...
                            embedding_bytes = state_row[
                                embedding_col_mapping[state_key_type]
                            ]
                            if embedding_bytes:
                                state_embedding = np.frombuffer(
        ...
        # If we have state IDs, fetch windows around those states
            ...
            for state_id, trajectory_id in zip(state_ids, trajectory_ids):
                ...
                if state_row:
                    ...
                    self.state_cursor.execute(
                        "SELECT id FROM states WHERE trajectory_id = ?",
                        (trajectory_id,),
                    )
