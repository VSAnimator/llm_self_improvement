    ...
    while i < len(lines):
        ...
        if line.startswith("Initial observation: "):
            ...
            while i < len(lines) and not (
                ...
                or lines[i].startswith("Selected action:")
                or lines[i].startswith("Obs:")
                or lines[i].startswith("Initial observation:")
                or lines[i].startswith("Reward:")
            ):
...
def ingest_wordcraft_log(log_file_path, db):
    ...
    # Parse log content
    goal, category, plan, obs_list, reasoning_list, act_list, rewards = (
        parse_wordcraft_log(log_content)
    )
...
if __name__ == "__main__":
    ...
    log_files = glob.glob(
        "./src/llm_agent/starter_data/example_files/wordcraft_examples/*.txt"
    )
    ingest_multiple_logs(log_files, "./data/starter/wordcraft/learning.db")
