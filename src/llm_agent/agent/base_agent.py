...
class BaseAgent:
    ...
    def __init__(self, llm, db, env, config):
        ...
        self.category: Optional[str] = (
            env.category if hasattr(env, "category") else None
        )
        self.action_space: Optional[str] = (
            env.get_action_space() if hasattr(env, "get_action_space") else None
        )
    ...
    def _create_conversation(
        self, keys: List[str], available_actions: List[Action]
    ) -> List[Dict]:
        """
        ...
        for key in keys:
            if key == "goal":
                ...
                trajectory_dict[key] = [
                    self.observation_history[i].structured
                    for i in range(len(self.observation_history))
                ]
            elif key == "reasoning":
                trajectory_dict[key] = [
                    self.reasoning_history[i]
                    for i in range(len(self.reasoning_history))
                ]
            elif key == "action":
                trajectory_dict[key] = [
                    self.action_history[i].text for i in range(len(self.action_history))
                ]
            elif key == "available_actions":
                trajectory_dict[key] = repr(
                    [f"{i+1}. {a.text}" for i, a in enumerate(available_actions)]
                )
...
Your actions should be clear, concise, and directly executable in the environment."""
        ...
        if self.goal:
            initial_obs = (
                self.observation_history[0].structured
    ...
    def _roll_up_trajectory(self, entries):
        ...
        if any(k in value_type for k in interleaved_keys):
            for i in range(len(entries)):
                ...
                if (
                    self.category == "sql"
                    and "action" in entries[i]
                    and len(entries[i]["action"]) >= 2
                ):
    ...
    def _retrieve_in_context_data(
        ...
        """
        success_entries, failure_entries = self.db.retrieve_similar_entries(
            key_type,
            key,
            outcome=outcome,
            k=k,
            window=window,
            filtered_environment_id=filtered_environment_id,
        )
        if self.f:
        ...
        success_entries = [
            {k: v for k, v in entry.items() if k in value_type}
    ...
    def retrieve_state_data(
        ...
        return self._retrieve_in_context_data(
            key_type,
            key,
            value_types,
            outcome,
    ...
    def retrieve_trajectory_data(
        self, key_types, keys, value_types, outcome, k, filtered_environment_id="self"
    ) -> List[Dict]:
        """
    ...
    async def reason(
        self, observation: Observation, available_actions, in_context_data=None
    ) -> List[Dict]:
        """
        ...
        conversation.append(
            {
                "role": "user",
                "content": self._create_conversation(
                    ["goal", "plan", "observation", "reasoning", "action"],
                    available_actions,
                )
                + "reasoning: ",
            }
    ...
    async def reflect(self, in_context_data=None) -> List[Dict]:
        ...
        """
        user_prompt = self._create_conversation(
            ["goal", "plan", "observation", "reasoning", "action"], []
        )
    ...
    def clean_history(self):
        ...
        nothing_indices = [
            i
            for i in range(len(self.observation_history))
            if "Nothing happens" in self.observation_history[i].structured
        ]
