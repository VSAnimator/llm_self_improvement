...
class RAP(BaseAgent):
    ...
    async def choose_action(self, obs, valid_actions, log_file):
        ...
        if self.in_context_data is None:
            self.in_context_data = self.get_in_context_data(key_type=["goal", "category", "plan", "observation"], key=[self.goal, self.category, self.plan, obs.structured], value_type=["goal", "observation", "reasoning", "action"], outcome="winning", k=4, window=5)
