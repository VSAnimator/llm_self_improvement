...
class RAP(BaseAgent):
    ...
    async def choose_action(self, obs, valid_actions, log_file):
        ...
        if self.in_context_data is None:
            self.in_context_data = self.get_in_context_data(key_type=["goal", "category", "plan", "observation"], key=[self.goal, self.category, self.plan, obs.structured], value_type=["goal", "observation", "reasoning", "action"], k=3)
        reasoning = await self.reason(obs, valid_actions, in_context_data=self.in_context_data)
        self.in_context_data = self.get_in_context_data(key_type=["goal", "category", "plan", "reasoning"], key=[self.goal, self.category, self.plan, reasoning], value_type=["goal", "plan", "observation", "reasoning", "action"], k=4, window=5)
        #print("action_data", self.in_context_data)
        #input()
