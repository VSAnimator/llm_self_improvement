...
from intercode.envs import BashEnv, CTFEnv, SqlEnv
...
DEMO_MAP = {
    "bash": {
        ...
        "data_path": base_path + "nl2bash/nl2bash_fs_1.json",
    },
    "sql": {
        "env": SqlEnv,
        "image_name": "docker-env-sql-spider",
        "data_path": base_path + "sql/spider/ic_spider_dev.json",
        "preprocess": preprocess_sql,
    },
    "ctf": {
        "env": CTFEnv,
        "image_name": "intercode-ctf",
        "data_path": base_path + "ctf/ic_ctf.json",
        "preprocess": preprocess_ctf,
    },
}

from ..base_env import Action, BaseEnv, Observation
...
class InterCodeSqlEnv(BaseEnv):
    def __init__(self, config):
        ...
        data_path = (
            DEMO_MAP[demo]["data_path"] if "data_path" in DEMO_MAP[demo] else None
        )
        self.env = DEMO_MAP[demo]["env"](
            image_name, data_path=data_path, verbose=True, preprocess=preprocess_sql
        )
        ...
        if self.problem_id >= num_entries:
            raise ValueError(
                f"Problem id {self.problem_id} is out of range for train split"
            )
