...
class Task:
    ...
    def __eq__(self, other):
        ...
        return (
            ...
            and self.base_entities == other.base_entities
            and self.intermediate_entities == other.intermediate_entities
            and self.relevant_recipes == other.relevant_recipes
        )
...
class RecipeBook:
        ...
        for b in base_entities:
            if (
                ...
                sorted_recipes = sorted(
                    self.entity2recipes[b], key=lambda r: tuple(sorted(r.items()))
                )
    ...
    def _init_tasks_for_depth(self, max_depth=2):
        ...
        for d in self.depth2task:
            ...
            self.depth2task[d] = tuple(
                sorted(self.depth2task[d], key=lambda t: (t.goal, t.base_entities))
            )
    ...
    def _fill_recipe_entity_support(self):
        ...
        for e in missing_entities:
            ...
            recipes_for_entity = sorted(
                list(entity2recipes_test[e]), key=lambda r: tuple(sorted(r.items()))
            )
