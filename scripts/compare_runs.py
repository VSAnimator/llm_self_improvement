...
class RetrievalRunComparison:
        ...
        for run_folder in self.run_folders:
            ...
            if influence_path.exists():
                ...
                for key in self.influence_data[run_folder]:
                    ...
                    db.trajectory_cursor.execute(
                        f"SELECT environment_id, goal FROM trajectories WHERE id = {key}"
                    )
            ...
            if stats_path.exists():
                ...
                for key in self.example_stats[run_folder]:
                    swapped_key_data[key_swaps[key]] = self.example_stats[run_folder][
                        key
                    ]
    ...
    def find_differential_performance(self, min_usage=5, diff_threshold=0.3):
        ...
        """
        print(
            f"Finding examples with differential performance (threshold={diff_threshold})..."
        )
        ...
        for example_id in multi_run_examples:
            ...
            if (
                direct_diff >= diff_threshold
                or cascade_diff >= diff_threshold
                or combined_diff >= diff_threshold
            ):

                best_run = example_df.loc[
                    example_df["direct_success_rate"].idxmax(), "run"
                ]
                worst_run = example_df.loc[
                    example_df["direct_success_rate"].idxmin(), "run"
                ]
        ...
        if not diff_df.empty:
            ...
            # Save results
            diff_df.to_csv(
                self.output_dir / "differential_performance.csv", index=False
            )
        else:
    ...
    def analyze_run_differences(self):
        ...
        if not param_df.empty:
            # Save results
            param_df.to_json(
                self.output_dir / "parameter_differences.json",
                orient="records",
                indent=2,
            )
            ...
            with open(self.output_dir / "parameter_frequency.txt", "w") as f:
                f.write(
                    "Parameters that most frequently differ between best and worst runs:\n"
                )
                f.write(
                    "=================================================================\n\n"
                )
    ...
    def generate_report(self):
        ...
        with open(self.output_dir / "comparison_report.md", "w") as f:
            ...
            f.write("## Overview\n\n")
            f.write(
                f"This report compares example performance across {len(self.run_folders)} different runs.\n\n"
            )
            ...
            if not diff_df.empty:
                ...
                f.write(
                    "The following examples showed significantly different performance across runs:\n\n"
                )

                f.write(
                    "| Example ID | Direct Diff | Best Run | Worst Run | Best Rate | Worst Rate |\n"
                )
                f.write(
                    "|------------|------------|----------|-----------|-----------|------------|\n"
                )
                ...
                for _, row in diff_df.iterrows():
                    f.write(
                        f"| {row['example_id']} | {row['direct_diff']:.3f} | {row['best_run']} | {row['worst_run']} | {row['max_direct']:.3f} | {row['min_direct']:.3f} |\n"
                    )
                ...
                f.write(
                    "Individual performance charts were generated for the top examples with differential performance.\n\n"
                )
                f.write("![Heatmap](top_examples_heatmap.png)\n\n")
            else:
                f.write("## No Significant Performance Differences\n\n")
                f.write(
                    "No examples showed significant performance differences across the analyzed runs.\n\n"
                )
            ...
            if param_analysis_file.exists():
                ...
                f.write(
                    "The following parameters most frequently differed between best and worst runs:\n\n"
                )

                with open(
                    self.output_dir / "parameter_frequency.txt", "r"
                ) as param_file:
                ...
                f.write(
                    "This suggests these parameters may have the most impact on example effectiveness.\n\n"
                )
            ...
            if not diff_df.empty:
                ...
                f.write(
                    f"Example {top_example} showed the largest performance difference across runs, "
                )
                f.write(
                    f"with a direct success rate difference of {diff_df.iloc[0]['direct_diff']:.3f}.\n\n"
                )
                f.write(
                    "Consider analyzing these high-differential examples more closely to understand why their performance varies so significantly across different configurations.\n"
                )
            else:
                f.write(
                    "Examples showed consistent performance across the analyzed runs. This suggests that the effectiveness of examples is relatively stable across the tested hyperparameter configurations.\n"
                )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Compare influence scores across multiple retrieval runs"
    )
    parser.add_argument("base_dir", help="Base directory containing all run folders")
    parser.add_argument(
        "--runs", nargs="+", required=True, help="List of run folder names to compare"
    )
    parser.add_argument(
        "--db_paths", nargs="+", required=True, help="List of database paths to compare"
    )
    parser.add_argument(
        "--output",
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Minimum usage count to consider an example",
    )
    parser.add_argument(
        "--diff-threshold",
        type=float,
        default=0.3,
        help="Minimum difference in success rate to be significant",
    )
    parser.add_argument(
        "--max-task-id", type=int, default=1200, help="Maximum task ID to analyze"
    )

    args = parser.parse_args()

    comparator = RetrievalRunComparison(
        args.base_dir, args.runs, args.db_paths, args.output, args.max_task_id
    )
