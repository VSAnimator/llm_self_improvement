...
for folder in sorted(txt_folders):
    ...
    episode_files = [
        f for f in episode_files if f.split("/")[-1].split(".")[0].isdigit()
    ]
    # Filter out files with names >= 4800
    episode_files = [
        f for f in episode_files if int(f.split("/")[-1].split(".")[0]) < 4800
    ]
    ...
    episode_files = sorted(
        episode_files, key=lambda x: int(x.split("/")[-1].split(".")[0])
    )
    for episode_file in episode_files:
        with open(episode_file, "r") as f:
            ...
            final_reward = (
                float(final_reward) if final_reward.replace(".", "", 1).isdigit() else 0
            )
            ...
            for i, line in enumerate(lines):
                if (
                    "Step 20 of 20" in line
                    or "Step 30 of 30" in line
                    or "Reward: 1" in line
                ):
            ...
            success_attempts_dict[success_attempts] = (
                success_attempts_dict.get(success_attempts, 0) + 1
            )
    ...
    for i in range(0, len(final_rewards), 200):
        print(
            f"Average success rate over {i+200} episodes:",
            np.mean(np.array(final_rewards[i : i + 200]) > 0.99),
        )

    continue
    print(
        "Success dict",
        dict(sorted(success_count_dict.items(), key=lambda item: item[0])),
    )
    print(
        "Failure dict",
        dict(sorted(failure_count_dict.items(), key=lambda item: -1 * item[0])),
    )

    print(
        "Average success per attempt",
        {k: sum(v) / len(v) for k, v in attempt_dict.items()},
    )
    average_success_rates[folder] = {
        k: sum(v) / len(v) for k, v in attempt_dict.items()
    }

    print(
        "Overall average success",
        sum(sum(v) / len(v) for v in attempt_dict.values())
        / len(attempt_dict.values()),
    )
    ...
    weighted_sum = sum(
        (1 / k if k > 0 else 0) * v for k, v in success_attempts_dict.items()
    )
    ...
    """
    print(
        f"Last 100 episode average success rate:",
        np.mean(np.array(final_rewards[-100:]) == 1),
    )
...
for folder in average_success_rates:
    for i in range(1, 6):
        ...
        if i not in cumulative_success_rates[folder]:
            cumulative_success_rates[folder][i] = cumulative_success_rates[folder][
                i - 1
            ]
...
for folder in average_success_rates:
    ...
    cumulative_success_rates[folder] = dict(
        sorted(cumulative_success_rates[folder].items())
    )
...
for folder in average_success_rates:
    ...
    if label == "alfworld":
        ...
        label = "react"
    ax1.plot(
        list(average_success_rates[folder].keys()),
...
for folder in cumulative_success_rates:
    ...
    if label == "alfworld":
        ...
        label = "react"
    ax2.plot(
        list(cumulative_success_rates[folder].keys()),
