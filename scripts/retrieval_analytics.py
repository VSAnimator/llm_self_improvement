...
class RetrievalAnalytics:
    def __init__(self, logs_dir):
        ...
        self.example_tree = (
            nx.DiGraph()
        )  # Tracks which examples led to which new examples
        self.example_success_count = (
            Counter()
        )  # How many times each example was used in successful tasks
        self.example_failure_count = (
            Counter()
        )  # How many times each example was used in failed tasks
        self.example_usage_count = (
            Counter()
        )  # How many times each example was retrieved
        self.example_task_types = defaultdict(
            Counter
        )  # Maps examples to the tasks they helped with
    ...
    def parse_log_file(self, file_path, example_id):
        ...
        for match in success_matches:
            if match.strip():
                ...
                for retrieved_id in ids:
                    ...
                    if is_successful:
                        self.example_success_count[retrieved_id] += 1 / len(
                            success_matches
                        )
                    else:
                        self.example_failure_count[retrieved_id] += 1 / len(
                            success_matches
                        )
                    self.example_task_types[retrieved_id][goal] += 1 / len(
                        success_matches
                    )
        ...
        for retrieved_id in all_retrieved_ids:
            if retrieved_id not in self.example_tree:
                self.example_tree.add_node(
                    retrieved_id, type="generated", success_rate=0, usage_count=0
                )
    ...
    def analyze_logs(self):
        ...
        log_files = sorted(
            [f for f in os.listdir(self.logs_dir) if f.endswith(".txt")],
        ...
        for file_name in tqdm(log_files, desc="Processing log files"):
            ...
            example_id = (
                18 + int(file_name.split(".")[0]) + 1
            )  # Base 18 examples + file number + 1
        ...
        for node in self.example_tree.nodes():
            ...
            if usage > 0:
                print(
                    f"Updating node {node} with success rate {success / usage}",
                    "usage",
                    usage,
                    "success",
                    success,
        ...
        for node in self.example_tree.nodes():
            ...
            if descendants:
                descendant_success_rates = [
                    self.example_tree.nodes[d]["success_rate"]
                    for d in descendants
                    if "usage_count" in self.example_tree.nodes[d]
                    and self.example_tree.nodes[d]["usage_count"] > 0
                ]
                cascade_rate = (
                    np.mean(descendant_success_rates) if descendant_success_rates else 0
                )
            else:
                cascade_rate = 0
    ...
    def generate_reports(self):
        ...
        return {"example_stats": example_stats, "influence_scores": influence_scores}
    ...
    def visualize_example_tree(self, output_path="example_tree.png", min_usage=1):
        ...
        nodes_to_remove = [
            n for n, attr in G.nodes(data=True) if attr["usage_count"] < min_usage
        ]
    ...
    def visualize_top_performers(self, output_path="top_performers.png", top_n=20):
        ...
        sorted_examples = sorted(
            influence_scores.items(), key=lambda x: x[1]["combined_score"], reverse=True
        )[:top_n]
    ...
    def run_analysis(self, output_dir="results"):
        ...
        with open(os.path.join(output_dir, "summary.txt"), "w") as f:
            ...
            # Success rates
            avg_success_rate = (
                sum(ex["success_rate"] for ex in reports["example_stats"].values())
                / total_examples
            )
            ...
            top_usage = sorted(
                reports["example_stats"].items(),
                key=lambda x: x[1]["usage_count"],
                reverse=True,
            )[:10]
            for ex_id, stats in top_usage:
                f.write(
                    f"  Example {ex_id}: Used {stats['usage_count']} times, {stats['success_rate']:.2f} success rate\n"
                )
            f.write("\n")
            ...
            top_success = sorted(
                [
                    item
                    for item in reports["example_stats"].items()
                    if item[1]["usage_count"] >= 5
                ],
            ...
            for ex_id, stats in top_success:
                f.write(
                    f"  Example {ex_id}: {stats['success_rate']:.2f} success rate, used {stats['usage_count']} times\n"
                )
            f.write("\n")
            ...
            top_influence = sorted(
                reports["influence_scores"].items(),
                key=lambda x: x[1]["combined_score"],
                reverse=True,
            )[:10]
            for ex_id, scores in top_influence:
                f.write(
                    f"  Example {ex_id}: Combined score {scores['combined_score']:.2f}, "
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output", default="results", help="Output directory for analysis results"
    )
