...
def calculate_accuracy(folder_path, segment_size=100):
    ...
    episode_files = [
        f for f in episode_files if os.path.basename(f).split(".")[0].isdigit()
    ]
    ...
    episode_files = sorted(
        episode_files, key=lambda x: int(os.path.basename(x).split(".")[0])
    )

    # Get the last 100 files (or all if less than 100)
    last_files = (
        episode_files[-segment_size:]
        if len(episode_files) > segment_size
        else episode_files
    )
...
def calculate_pass_at_k(folder_paths):
    ...
    if not base_files:
        return {k: 0.0 for k in range(1, len(folder_paths) + 1)}, {
            k: 0.0 for k in range(1, len(folder_paths) + 1)
        }

    # Sort files numerically
    base_files = sorted(
        base_files, key=lambda x: int(os.path.basename(x).split(".")[0])
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Calculate accuracy on the last 100 tasks in a folder."
    )
    parser.add_argument(
        "folder", type=str, help="Path to the folder containing episode files"
    )
